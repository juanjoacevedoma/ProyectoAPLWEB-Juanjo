<?php
$pageTitle = 'Control Dashboard | Hardware Hub Pro';
$currentPage = 'index';
...
<main class="container">
    <!-- Dashboard Header -->
    <header class="header-meta reveal" style="margin-bottom: 5rem; display: flex; justify-content: space-between; align-items: flex-end;">
        <div>
            <span style="color: var(--primary); font-weight: 800; text-transform: uppercase; letter-spacing: 0.2em; font-size: 0.8rem;">SISTEMA DE CONTROL DE ACTIVOS</span>
    ...
    <div class="stats-grid reveal">
        <div class="stat-card glass">
            ...
            <div style="height: 4px; background: rgba(99, 102, 241, 0.2); width: 100%; margin-top: 1.5rem; border-radius: 2px; overflow: hidden;">
    ...
    <div class="search-container reveal" style="margin-bottom: 3rem;">
        <div style="display: flex; flex: 1; align-items: center; padding-left: 1.5rem;">
            ...
            <input type="text" id="searchInput" class="search-input" placeholder="Buscar por nombre, fabricante o SKU...">
        </div>
        <div style="width: 1px; background: var(--border); margin: 0.5rem 0;"></div>
        <select id="filterCategory" style="background: transparent; border: none; color: white; padding: 0 2rem; font-weight: 600; cursor: pointer; outline: none;">
            <option value="all">Todas las Categorías</option>
            <?php foreach ($categoriasArr as $cat): ?>
                <option value="<?php echo htmlspecialchars($cat); ?>" style="background: var(--bg-alt);"><?php echo htmlspecialchars($cat); ?></option>
            <?php endforeach; ?>
        </select>
    </div>
            ...
            <?php foreach ($componentes as $comp): ?>
                <div class="hardware-card glass reveal inventory-item" 
                    ...
                    <div class="card-img-container">
                        ...
                        <?php else: ?>
                            <div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%);">
                                <span style="font-size: 2.5rem; opacity: 0.3; filter: grayscale(1);">🔌</span>
                            </div>
                        <?php endif; ?>
                        
                        <!-- Quick Badge -->
                        <div style="position: absolute; top: 1rem; right: 1rem; padding: 0.4rem 0.8rem; border-radius: 8px; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.1); font-size: 0.7rem; font-weight: 800; color: white;">
                    ...
                    <div class="card-body">
                        ...
                        <div style="margin-top: 1rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
                            <span style="font-size: 0.75rem; padding: 0.3rem 0.7rem; border-radius: 6px; background: rgba(99, 102, 241, 0.1); color: var(--primary); font-weight: 700;">
                                <?php echo htmlspecialchars($comp['marca_nombre']); ?>
                            </span>
                            <?php if ($comp['stock'] < 5): ?>
                                <span style="font-size: 0.75rem; padding: 0.3rem 0.7rem; border-radius: 6px; background: rgba(239, 68, 68, 0.1); color: #f87171; font-weight: 700;">
                                    Stock Crítico
                                </span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="card-footer">
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                <a href="detalle.php?id=<?php echo $comp['id']; ?>" class="btn" style="padding: 0.6rem; border: 1px solid var(--border); background: rgba(255,255,255,0.02);" title="Ver Detalles">
                                    👁️
                                </a>
                                <button onclick="confirmDelete(<?php echo $comp['id']; ?>, '<?php echo addslashes($comp['nombre']); ?>')" 
                                        class="btn" style="padding: 0.6rem; border: 1px solid rgba(239, 68, 68, 0.2); background: rgba(239, 68, 68, 0.05);" title="Eliminar">
                                    🗑️
                                </button>
                            </div>
                        </div>
